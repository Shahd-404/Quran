-- Migration: ensure_current_reading_assignment
-- Timestamped: 2026-07-26T18:00:00
...
BEGIN
  ...
  SELECT id, local_date, target_pages, (SELECT count(*) FROM public.reading_sessions rs WHERE rs.daily_assignment_id = da.id) as sess_count
  ...
  IF v_assignment_id IS NOT NULL THEN
    ...
    created_now := false;
    carried_over := true;
    target_pages := COALESCE(v_assigned, 0)::smallint;
    session_count := COALESCE(v_sessions_to_use, 0)::smallint;
    RETURN NEXT;
    RETURN;
  END IF;
  ...
  SELECT id, local_date, target_pages, (SELECT count(*) FROM public.reading_sessions rs WHERE rs.daily_assignment_id = da.id)
  INTO v_assignment_id, v_local_date, v_assigned, v_sessions_to_use
  FROM public.daily_assignments da
  ...
  created_now := true;
  carried_over := false;
  target_pages := v_assigned::smallint;
  session_count := v_sessions_to_use::smallint;
  RETURN NEXT;
  RETURN;
