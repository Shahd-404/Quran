-- Install the scheduled reminder invocation after its Vault values exist.
-- Values remain in Vault and are resolved only when each Cron execution runs.
...
BEGIN
  ...
  -- Replace only this application's stable job. Unrelated Cron jobs are kept.
