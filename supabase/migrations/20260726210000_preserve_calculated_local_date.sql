-- Migration: preserve the calculated local date when no assignment exists yet
-- Timestamped: 2026-07-26T21:00:00
...
BEGIN
  ...
  SELECT da.id, da.local_date, da.target_pages, (SELECT count(*) FROM public.reading_sessions AS rs WHERE rs.daily_assignment_id = da.id) AS sess_count
  ...
  -- Load active plan
  SELECT rp.id, rp.timezone, rp.current_unread_page, rp.daily_pages, rp.sessions_per_day, rp.effective_from
  ...
  WHERE rp.user_id = v_uid AND rp.status = 'active'
  LIMIT 1;
  ...
  WHERE k.user_id = v_uid
    AND k.reading_plan_id = v_plan_id
    AND k.status = 'active'
  LIMIT 1;
  ...
  -- Load schedule times
  SELECT pg_catalog.array_agg(pst.scheduled_time ORDER BY pst.session_order)
  INTO v_sched_times
  ...
  SELECT da.id, da.target_pages, (SELECT count(*) FROM public.reading_sessions AS rs WHERE rs.daily_assignment_id = da.id)
  INTO v_assignment_id, v_assigned, v_sessions_to_use
  FROM public.daily_assignments AS da
  ...
  INSERT INTO public.daily_assignments AS da(user_id, reading_plan_id, khatma_id, local_date, timezone, target_pages, status)
  VALUES (v_uid, v_plan_id, v_khatma_id, v_local_date, v_plan_timezone, v_assigned, 'pending')
  RETURNING da.id INTO v_assignment_id;
