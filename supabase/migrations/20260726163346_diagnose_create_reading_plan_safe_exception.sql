-- Diagnostic migration: preserve business errors and log unexpected SQL failures for create_reading_plan.
...
BEGIN
  ...
  SELECT COALESCE(MAX(cycle_number),0) + 1 INTO v_cycle_num FROM public.khatmas WHERE user_id = v_uid;
...
EXCEPTION WHEN others THEN
  ...
  RAISE LOG
    'create_reading_plan failure user=% state=% message=% detail=% hint=% context=%',
    v_uid,
    v_sqlstate,
    v_message,
    COALESCE(v_detail, ''),
  ...
  ELSIF v_message LIKE '%PROFILE_NOT_FOUND%' THEN
    RAISE EXCEPTION 'PROFILE_NOT_FOUND' USING ERRCODE = 'P0001';
  ELSIF v_message LIKE '%ACTIVE_PLAN_EXISTS%' THEN
    RAISE EXCEPTION 'ACTIVE_PLAN_EXISTS' USING ERRCODE = 'P0001';
  ELSIF v_message LIKE '%idx_khatmas_unique_active_per_user%' THEN
    RAISE EXCEPTION 'ACTIVE_PLAN_EXISTS' USING ERRCODE = 'P0001';
  ELSIF v_message LIKE '%INVALID_START_PAGE%' THEN
    RAISE EXCEPTION 'INVALID_START_PAGE' USING ERRCODE = 'P0001';
  ELSIF v_message LIKE '%INVALID_DAILY_PAGES%' THEN
    RAISE EXCEPTION 'INVALID_DAILY_PAGES' USING ERRCODE = 'P0001';
  ELSIF v_message LIKE '%INVALID_SESSIONS%' THEN
    RAISE EXCEPTION 'INVALID_SESSIONS' USING ERRCODE = 'P0001';
  ELSIF v_message LIKE '%INVALID_SCHEDULE%' THEN
    RAISE EXCEPTION 'INVALID_SCHEDULE' USING ERRCODE = 'P0001';
  ELSIF v_message LIKE '%INVALID_TIMEZONE%' THEN
    RAISE EXCEPTION 'INVALID_TIMEZONE' USING ERRCODE = 'P0001';
  ELSIF v_message LIKE '%INVALID_EFFECTIVE_DATE%' THEN
    RAISE EXCEPTION 'INVALID_EFFECTIVE_DATE' USING ERRCODE = 'P0001';
  ELSE
