...
describe('shared Service Worker', () => {
  ...
  it('deletes only old Wird-owned caches', () => {
    ...
    expect(source).toContain("const QCF_FONT_CACHE = 'wird-qcf-v2-fonts-v2'")
  ...
  it('keeps only complete official QCF fonts for at most seven days', () => {
    ...
    expect(source).toContain('bytes[0] === 0x77')
    expect(source).toContain('bytes[1] === 0x4f')
    expect(source).toContain('bytes[2] === 0x46')
    expect(source).toContain('bytes[3] === 0x32')
  })
})
